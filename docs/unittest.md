...
| `enum` | `enum.py:924:21`: IntFlag reports members in Flag; plain function.__get__ is missing, so descriptor classification treats ordinary methods as members. |
...
Object initialization now exposes a real runtime-cached wrapper descriptor and
bound method-wrapper values. Tests execute direct and bound calls, `__get__`,
inherited initialization, `super`, explicit wrapper assignment, and excess
...
`str.join` now has a real method descriptor, native bound-method metadata, and
receiver-checked direct/instance/__get__ calls into the existing resumable join
implementation. Tests include generator input and binding/argument failures.

`dict.fromkeys` now runs through a native classmethod descriptor with real class
binding. Tests check direct/instance/__get__ access, bad owners, iterator and key
...
Concrete class unions now support flattening, deduplication, None normalization,
read-only metadata, representation, order-independent equality/hash, and lazy
class checks. Executable type/union slots preserve metaclass and reflected-method
precedence. Custom metaclass equality/hashing, aliases, type parameters, and
broader Union construction/substitution remain unsupported. Unchanged types now
gets past UnionType discovery.

The globals() builtin now exposes the actual defining module dictionary through
module, function, class, comprehension, and generator frames. Mutations share the
existing module/function/frame namespace paths. Unchanged types now imports and
...
Python callbacks. Dictionary subclasses now retain native storage, inherit cached
native descriptors after Python MRO lookup, and run ordinary __new__/__init__
continuations. Tests cover subclass overrides, explicit slot bypass, live views,
...
non-docstring expressions. Runtime function.__doc__ now returns stable prepared
text or None, supports per-function replacement/deletion, and exposes the actual
member descriptor. Invalid encoded metadata is rejected before execution. The
...
complete compiler dumps also cover generic classes. Dictionary-subclass namespace
callbacks now execute for reads, writes, deletes, closure fallback, and compiler
class-cell/original-base publication. The original mapping reaches the metaclass
and retained frame locals; native type construction copies its storage. Tests
cover callback ordering, ignored mutation returns, and exception propagation.
Enum now reaches its actual member-name validation and fails on string %.

Positional string percent formatting now executes %s/%r/%a/%% with single or
tuple arguments, actual Python conversion callbacks, error ordering, and native
slot descriptors. Large immediate conversions use a loop. Numeric fields,
mapping keys, and formatting flags/width/precision remain later work. Enum gets
past private-name formatting and now requires native set intersection.

Native set/frozenset union, intersection, difference, and symmetric difference
now execute, with left-kind results, mutable in-place identity, representative
...
Native and ordinary user classes plus plain Python/native functions now work as
identity keys in dicts, sets, and nested immutable keys. Metaclass equality/hash
overrides remain explicitly rejected rather than silently ignored. Tests keep
same-name classes and closures distinct and check stable lookups. Enum now
reaches singleton/root allocator selection.

NoneType, EllipsisType, and NotImplementedType now have actual singleton
constructors and static allocation functions. Object.__new__ allocates ordinary
instances without initialization, validates layout and argument safety, and
honors abstract allocation. Tests cover inherited/static access and errors;
unsupported native allocators are not replaced by object markers. Enum advances
to object.__format__ inspection.

Object.__format__ now executes empty-spec str conversion and validates inputs
and callback results. Tests cover direct, inherited, and super calls; native
...
Object reduction now executes through unchanged copyreg for ordinary objects
and dictionary subtypes. Tests cover protocol conversion, class __reduce__
overrides, descriptor failures, validated __getnewargs__/__getnewargs_ex__,
state callbacks, lazy items, and reconstruction with the actual Python helpers.
Unknown native layouts and nonempty slot state remain guarded. State discovery
also calls the real slot helper for native types without a cache and distinguishes
...
Set.pop now removes a real stored member with identity preservation, argument
checks, empty-set errors, and iterator mutation detection. It is a native method
...
String subclasses now retain actual immutable text, construct through native
allocation plus Python initialization, and inherit executable native protocols
and text methods. Tests cover override and explicit-bypass behavior, native text
arguments, empty/nonempty formatting, reflected comparisons/percent formatting,
and mixed string/Python C3 order. Encoding forms and scalar subtype keys/default
reduction remain unsupported. Enum reaches FlagBoundary unpacking at line 1398:
class objects need metaclass iteration, and enum member descriptors still need
actual __set_name__ callbacks during type construction. Importing enum would not
be enough without checking member construction and behavior.

Descriptor naming now runs class-special __set_name__ on a snapshot before
native type construction returns. Tests cover ordering, namespace mutation,
metaclass timing, class-cell availability, descriptor-valued hooks, ignored
instance replacements, exception identity/notes, and real property naming with
subclass delegation. Native completions loop; Python callbacks use VM frames.
...
Fixed and starred unpacking now consume arbitrary native, generator, user, and
metaclass iterables through the VM. Tests verify bounded excess-item consumption,
starred remainder iteration, unchanged targets on failure, and callback errors.
Enum gets through FlagBoundary unpacking and reaches IntFlag creation, where
Flag incorrectly has members: _is_descriptor cannot see a plain function's
missing executable __get__ attribute. Implement the binding operation itself,
not a marker that merely makes the classifier return true.

The table lists first failures, not complete missing-feature lists. Next,
implement function descriptor binding, then follow later Enum
