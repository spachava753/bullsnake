...
keeping setup state on the operand stack. An `as` binding uses resolver-selected
...
exceptional entry that retains the pending exception. The exceptional copy
enters a handled-exception scope, so bare `raise` reraises that pending value,
and leaves the scope before reraising or making another transfer. Returns use
