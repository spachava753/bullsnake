...
`POP_TOP`, `COPY`, `SWAP`, fixed `BUILD_TUPLE`, `BUILD_LIST`, and
`UNPACK_SEQUENCE`, scalar `UNARY_OP`, selected integer `BINARY_OP`, and scalar
`COMPARE_OP` variants, absolute `JUMP`, both pop-and-test jumps, both short-circuit-or-pop jumps, and
...
and allocate heap-backed sequence values. Exact tuple and list unpacking pushes
elements in reverse so nested assignment targets store left to right. Arity
mismatches raise `ValueError`; other values raise `TypeError`. Tuple and list
truth depends on length. Starred construction and unpacking, indexing,
iteration, mutation, and cyclic representations are not implemented.
...
scalar and sequence truth testing, numeric unary operations, selected arbitrary-
precision integer binary operations, boolean short-circuiting, conditional expressions, conditional statements,
