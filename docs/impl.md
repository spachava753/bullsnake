...
- structural matching with literal and dotted-value tests, capture, wildcard,
  AS and OR patterns, guards, and fixed or starred tuple/list sequences
...
names. Deferred annotation bodies are also children and do not run during an
ordinary function definition or call.
...
asynchronous comprehensions, `async for`, `async with`, mapping and class
patterns, and coroutines. Unsupported AST forms return compiler errors; they
...
- no asynchronous comprehensions, coroutines, async execution, or Python
  threads
- no asynchronous context managers or mapping and class patterns
