...
func TestOpcodeFormattingAndStackEffects(t *testing.T) {
	...
	flags := Optimized | NewLocals | VarArgs | Nested | Generator | Coroutine | AsyncGenerator
	if got := flags.String(); got != "Optimized, NewLocals, VarArgs, Nested, Generator, Coroutine, AsyncGenerator" {
	...
	if got := (Instruction{Opcode: AsyncGenWrap}).String(); got != "ASYNC_GEN_WRAP" {
		t.Fatalf("async generator wrap instruction = %q", got)
	}
	...
	if got := (Instruction{Opcode: SetTypeVarDefault}).String(); got != "SET_TYPE_VAR_DEFAULT" {
		t.Fatalf("type variable default instruction = %q", got)
	}
	if got := SetTypeVarDefault.StackEffect(0); got != -1 {
		t.Fatalf("SET_TYPE_VAR_DEFAULT stack effect = %d, want -1", got)
	}
	if got := (Instruction{Opcode: SetFunctionTypeParameters}).String(); got != "SET_FUNCTION_TYPE_PARAMETERS" {
		t.Fatalf("function type parameter instruction = %q", got)
	}
