...
func TestGenericFunctionCompilerBoundary(t *testing.T) {
	module, err := parser.Parse("input.py", "def generic[T = Default]():\n    pass\n")
	if err != nil {
		t.Fatal(err)
	}
	table, err := resolver.Resolve("input.py", module)
	...
	if compileErr.Message != "generic function TypeVar defaults and variadic type parameters are not compiled" {
