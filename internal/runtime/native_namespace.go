...
func (runtime *Runtime) nativeNamespace(class *nativeTypeValue) *dictValue {
	...
	if class == objectNativeType {
		dictionary.set(&stringValue{value: "__init_subclass__"}, &nativeDescriptorValue{class: class, kind: nativeClassMethodDescriptor, name: "__init_subclass__", call: executeObjectInitSubclass})
	...
	addContextDescriptors(class, dictionary)
