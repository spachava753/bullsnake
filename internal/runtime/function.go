...
func executeFunctionCall(
	...
	case *nativeDescriptorValue:
		return executeNativeDescriptorCall(caller, instruction, base, callable, arguments, keywords)
	case *boundNativeDescriptorValue:
		bound := append([]Value{callable.self}, arguments...)
		return executeNativeDescriptorCall(caller, instruction, base, callable.descriptor, bound, keywords)
	...
	case *stringJoinMethod:
		return executeStringJoinCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *listRemoveMethod:
		return executeListRemoveCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *dictionaryClearMethod:
		return executeDictionaryClearCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *dictionaryGetMethod:
		return executeDictionaryGetCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *dictionaryKeysMethod:
		return executeDictionaryKeysCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *dictionaryValuesMethod:
		return executeDictionaryValuesCall(
			caller,
			instruction,
			base,
			callable,
	...
	case *setDiscardMethod:
		return executeSetDiscardCall(
			caller,
			instruction,
			base,
			callable,
...
func isCallableValue(value Value) bool {
	...
	case *instanceValue:
		...
		return found
	case *classWeakReference,
		*nativeDescriptorValue,
		*boundNativeDescriptorValue,
